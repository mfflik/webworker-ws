...
const MainTable: React.FC<{ worker: Worker | null }> = ({ worker }) => {
  ...
  const table = useReactTable({
    ...
    onSortingChange: (updater) => {
      ...
      worker?.postMessage({ type: "setSort", sortBy: nextSorting });
    },
    onColumnFiltersChange: (updater) => {
      const nextFilters = typeof updater === "function" ? updater(columnFilters) : updater;
      setColumnFilters(nextFilters);
      worker?.postMessage({ type: "setFilter", filters: nextFilters });
    },
  ...
  useEffect(() => {
    ...
    const handler = (e: MessageEvent) => {
      if (e.data.mode === "main") setPageResult(e.data);
    };
  ...
  const goToNextPage = () => {
    if (worker && pageResult.currentPage < pageResult.totalPages) {
      worker.postMessage({ type: "next" });
    }
  };
  const goToPreviousPage = () => {
    if (worker && pageResult.currentPage > 1) {
  ...
  const handleGotoPageSubmit = (e: React.FormEvent) => {
    ...
    if (worker && !isNaN(pageNum) && pageNum >= 1 && pageNum <= pageResult.totalPages) {
      worker.postMessage({ type: "goto", page: pageNum });
    }
    setGotoPageValue("");
  };
  const handlePageSizeChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (worker) {
      worker.postMessage({ type: "setItemsPerPage", perPage });
    }
  };
                    ...
                    {header.column.getCanFilter() && (
                      <div>
                        <input
                          ...
                          className="w-full mt-1 px-1 py-0.5 text-xs border text-black border-gray-300 rounded focus:ring"
                          style={{ minWidth: 0 }}
                        />
                      </div>
                    )}
